...
class ImageAnnotator:
    def __init__(self, predict_distance=50):
        self.img = None
        self.img_shape = None
        self.img_vis = None
        self.points = []
    ...
    def fit_polynomial(self):
        ...
        # 使用get_predict_vector的地方
        vector_pos, vector_dir = self.get_predict_vector(self.img_shape[0] + self.predict_distance)
        self.vector_pos = vector_pos
        self.vector_dir = vector_dir
        start_pos = (vector_pos[0], vector_pos[1])
        end_pos = (vector_pos[0] - int(vector_dir[0] * self.predict_distance),
                   vector_pos[1] - int(vector_dir[1] * self.predict_distance))
        # 类型转换
...
#     # 加载一张图片
#     img_path = r"C:\Users\Steven\PycharmProjects\re_rc2024\dataset\original\dataset_2024-04-02_13-33-31.jpg"  # 替换为你的图片路径
#     img = cv2.imread(img_path)
#     if img is not None:
#         annotator = ImageAnnotator()
...
if __name__ == "__main__":
    dataset_folder_path = r"C:\Users\Steven\PycharmProjects\re_rc2024\dataset\original"  # 替换为你的数据集文件夹路径
    target_csv_path = r"../dataset/data.cvs"  # 替换为你的目标CSV文件路径
    try:
