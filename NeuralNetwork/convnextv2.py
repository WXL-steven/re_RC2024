...
class ConvNeXtV2(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        self.head1 = nn.Linear(dims[-1], 16)
        self.head2 = nn.Linear(16, 16)
        self.head3 = nn.Linear(16, 16)
        self.head4 = nn.Linear(16, dims[-1])

        self.head = nn.Linear(dims[-1], num_classes)  # head layer
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            x = self.stages[i](x)
        return x  # 返回特征图，形状为 (N, C, H, W)

    def forward(self, x):
        x = self.forward_features(x)  # 获取特征图
        x = self.norm(x.mean([-2, -1]))  # 先进行全局平均池化，然后应用层归一化
        x = self.head1(x)  # 通过分类头进行分类
        x = self.head2(x)
        x = self.head3(x)
        x = self.head4(x)
        x = self.head(x)
...
def convnextv2_tiny(num_classes: int):
    ###########################################################################
    # name: ConvNeXt V2-T; resolution: (224x224); params: 28.6M; FLOPs: 4.47G #
    ###########################################################################
